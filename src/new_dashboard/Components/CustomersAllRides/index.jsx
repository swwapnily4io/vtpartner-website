...
const CustomerAllRides = () => {
  ...
  useEffect(() => {
    fetchRidesData();
  }, [customerId, activeTab, currentPage, searchQuery, startDate, endDate]);
  ...
  const fetchRidesData = async () => {
    try {
      ...
      const response = await axios.post(
        `${serverEndPoint}/get_customer_rides`,
        {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      ...
      "Driver Accepted": { color: "info", text: "Driver Accepted" },
      "Driver Arrived": { color: "primary", text: "Driver Arrived" },
      "Otp Verified": { color: "info", text: "OTP Verified" },
      "Start Trip": { color: "primary", text: "Trip Started" },
      "Reached Last Drop": { color: "info", text: "Reached Destination" },
      "Make Payment": { color: "warning", text: "Payment Pending" },
      "End Trip": { color: "success", text: "Completed" },
  ...
  const addMarkersAndRoute = (
    ...
    if (ride.multiple_drops > 0 && ride.drop_locations) {
      try {
        const dropLocations = JSON.parse(ride.drop_locations);
        if (Array.isArray(dropLocations) && dropLocations.length > 0) {
          // Add markers for each drop location
          ...
          // Create route with waypoints for multiple drops
          const waypoints = dropLocations.slice(0, -1).map((location) => ({
            location: {
          ...
          const finalDestination = {
            lat: parseFloat(lastDrop.lat) || 0,
            lng: parseFloat(lastDrop.lng) || 0,
          };
  ...
  const addSingleDestinationRoute = (
    map,
    ride,
    directionsService,
    directionsRenderer,
    ...
    const destinationLatLng = {
      lat: parseFloat(ride.destination_lat) || 0,
      lng: parseFloat(ride.destination_lng) || 0,
    };
  ...
  const getRideId = (ride) => {
    // For completed rides, use order_id, for others use booking_id
    if (!ride) return "N/A";
    return ride.order_id || ride.booking_id || "N/A";
  };

  const getRideType = (ride) => {
    if (!ride) return "N/A";
    return ride.order_id ? "Order" : "Booking";
  };
  ...
  return (
    <div>
      ...
      <Container fluid>
        <Row className="m-1">
          <Col xs={12}>
            <h4 className="main-title">Customer Rides - {customerName}</h4>
            <ul className="app-line-breadcrumbs mb-3">
              <li className="">
                <a href="#" className="f-s-14 f-w-500">
                  <span>
                    <i className="ph-duotone ph-users f-s-16"></i> Customers
                  </span>
                </a>
              </li>
              ...
              <CardBody>
                ...
                <div className="row mb-4">
                  <div className="col-md-4">
                    <Input
                      type="text"
                      placeholder="Search by driver name, pickup, or drop location..."
                      value={searchQuery}
                  ...
                  <table className="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                      {getCurrentRides().map((ride) => (
                        <tr key={getRideId(ride)}>
                          <td>
                            <div>
                              <strong>#{getRideId(ride)}</strong>
                              <br />
                          ...
                          <td>
                            {ride.multiple_drops > 0 ? (
                              <div>
                                <p className="mb-0 f-s-12 text-secondary">
                                  <strong>
                                    Multiple Drops ({ride.multiple_drops})
                                  </strong>
                                </p>
                                <small className="text-muted">
                                  {ride.drop_locations &&
                                  Array.isArray(JSON.parse(ride.drop_locations))
                                    ? JSON.parse(ride.drop_locations).length +
                                      " locations"
                                    : "1 location"}
                                </small>
                              </div>
      ...
      >
        <ModalHeader toggle={() => setMapModal(!mapModal)}>
          Route Map -{" "}
          {selectedRide
            ? `${getRideType(selectedRide)} #${getRideId(selectedRide)}`
            : "Loading..."}
        </ModalHeader>
        <ModalBody>
          {mapLoading ? (
              ...
              <div className="row">
                <div className="col-md-6">
                  ...
                  <p className="text-muted">
                    {selectedRide.pickup_address || "N/A"}
                  </p>
                </div>
                <div className="col-md-6">
                  <h6>
                    Drop Location{selectedRide.multiple_drops > 0 ? "s" : ""}:
                  </h6>
                  {selectedRide.multiple_drops > 0 &&
                  selectedRide.drop_locations ? (
                    <div>
                      {(() => {
                        try {
                          ...
                          return dropLocations.map((location, index) => (
                            <div key={index} className="mb-2">
                              <strong>Drop {index + 1}:</strong>
                              <p className="text-muted mb-1">
                                {location.address || "N/A"}
                              </p>
                              {selectedRide.drop_contacts &&
                                (() => {
                                  try {
        ...
        <ModalBody>
          {selectedCancelledRide && (
            <div>
              <Alert color="warning">
                <strong>
                  {getRideType(selectedCancelledRide)} #
                  {getRideId(selectedCancelledRide)}
                </strong>
              </Alert>
              <div className="mb-3">
              ...
              <div className="mb-3">
                ...
                <p className="text-muted">
                  {selectedCancelledRide.driver_name || "N/A"} (
                  {selectedCancelledRide.driver_mobile || "N/A"})
                </p>
              </div>
            </div>
          )}
